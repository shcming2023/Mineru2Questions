...
**状态**: 修订稿
**修订日期**: 2026-02-21
**作者**: Manus AI (基于实际项目对齐修订)

> **自包含声明**: 本文档是 Mineru2Questions 项目的唯一标准文档（取代 v4.0）。它基于实际项目状态进行了精确对齐，涵盖了项目的全部背景、目标、功能需求、技术规格和数据定义，不依赖于任何外部文档、历史对话或代码注释。任何与本文档冲突的外部信息，以本文档为准。
...
| 4.0 | 2026-02-21 | Manus AI | **章节预处理架构回归全文推理**。根据产品决策：(1) 将章节预处理从"自适应三轨混合架构"回归为"零筛选全文推理 + 两轮 LLM 校验"方案（`chapterPreprocess.ts`，原 v9.1），废弃 `chapterPreprocessV2.ts` 并移入 `archive/`；(2) 明确章节预处理失败时任务应直接失败，移除优雅降级回退；(3) 更新原则四，将优雅降级的范围限定在 LLM 输出解析等局部容错，不再包含章节预处理整体失败；(4) 更新路线图与架构描述 |
| **4.1** | **2026-02-21** | **Manus AI** | **基于实际项目状态对齐修订**：(1) 修正技术栈描述，实际使用 Express.js 非 Hono；(2) 修正数据库表结构，实际为 `extraction_tasks`、`page_processing_logs`；(3) 修正版本号同步，package.json 仍为 1.0.0；(4) 修正文件路径约定，基于实际目录结构；(5) 修正API路由描述，实际为 routers.ts；(6) 修正路线图完成状态，基于实际代码；(7) 澄清暂停/取消功能实现状态 |
...
**对于**需要将海量、非结构化的 PDF 教育文档（如教材、讲义、试卷、练习册，涵盖数学、语文、理化等多学科）转化为高质量、结构化数据的**数据工程师和 AI 产品团队**，**Mineru2Questions 是一个**高保真结构化内容解析引擎，**它能够**通过一个鲁棒的、可观测的"全文推理双 LLM 流水线"，精确地提取题目内容并还原其在原始文档中的章节归属关系。**不同于**那些仅做简单 OCR 或依赖脆弱规则的传统工具，**我们的产品**提供了工业级的可靠性和可追溯性，确保每一条提取数据的准确性和结构完整性，为下游智能题库、学习分析和 AI 助教等应用提供最坚实的数据基石。

### 1.2 项目演进历史

本项目基于 OpenDCAI/DataFlow 官方仓库（`PDF_VQA_extract_optimized_pipeline`）进行二次开发，经历了六个关键阶段。
...
**第六阶段：章节预处理回归全文推理 (v4.0)**

经产品决策明确：章节预处理的正确方案是**全文推理**（LLM 通读全文，推理原书真实目录结构），而非依赖代码/正则的三轨混合方案。核心理由：MinerU 解析结果中 `text_level` 等字段不可信赖，TOC 页面检测依赖 PDF 制作质量，代码/正则无法处理"无目录但有逻辑结构"的文档，只有全局视角才能推断章节连续性（检测断号）。因此将章节预处理回归 `chapterPreprocess.ts`（零筛选全文推理 + 两轮 LLM 校验），废弃 `chapterPreprocessV2.ts`，并明确章节预处理失败时任务直接失败（不做降级回退）。

**第七阶段：项目状态对齐与修订 (v4.1)**

基于当前项目实际状态进行全面对齐修订，确保PRD与实现完全一致：修正技术栈描述（Express.js）、版本号同步（package.json仍为1.0.0）、数据库表结构对齐（extraction_tasks、page_processing_logs）、文件路径约定更新、API路由描述修正、路线图完成状态同步，以及澄清暂停/取消功能的实现状态。
...
系统设计必须将"失败"作为一等公民来考虑，在每个环节都建立验证、重试、告警和回退机制，确保局部失败不会导致整个任务的灾难性崩溃。**然而，优雅降级不适用于核心前置步骤的整体失败。当章节预处理（`ChapterPreprocess`）失败时，任务应直接标记为失败，而非静默降级——因为章节归属是本系统的核心价值，使用无章节信息的抽取结果不满足产品目标。** 优雅降级的适用范围限定为：单个 LLM 输出的解析失败（可重试）、单个 Chunk 的抽取失败（可跳过并记录）等局部、可恢复的错误。
...
- **"暂停"/"继续"**: 仅在 `processing` 状态显示（**当前为空实现，详见 §4.9**）
...
> **v4.1 说明**: 两个页面保持并存，但在导航和 UI 中需明确差异化定位，避免用户困惑。

### 4.9 暂停/取消功能（Should-have，下一迭代）

**当前实现状态 (v4.1)**：
- **前端**: "暂停"/"继续"按钮已在 TaskDetail 页面存在
- **后端**: `pauseTaskProcessing` 和 `cancelTaskProcessing` 仅打印警告日志，**不实际暂停任务**
- **实现状态**: 已定义函数接口但功能为空，优先级 Should-have，纳入下一迭代计划（见 §8.2 P1-新增）
...
本系统的核心是一条由 7 个算子组成的**全文推理双 LLM 流水线**。章节预处理阶段使用长上下文 LLM 通读全文，以全局视角推理原书真实目录结构；题目抽取阶段使用视觉/通用 LLM 分块处理每个 Chunk。
...
└─────────┬───────────────┘
          ...
          ▼
┌─────────────────────────────────────────────────┐
│ ② ChapterPreprocess (全文推理，两轮 LLM 校验)    │
│                                                  │
│  格式化全文 → 第一轮 LLM 抽取章节 →              │
│  代码后处理（噪声过滤）→ 第二轮 LLM 自我校验     │
│                                                  │
│  失败 → 任务直接失败（无降级回退）               │
└─────────┬───────────────────────────────────────┘
          │
          ▼
┌─────────────────────────┐
│ ③ ChapterValidation     │ → chapter_flat_map.json
│    (验证与容错)          │
└─────────┬───────────────┘
          │
          ▼
...
> **章节预处理失败即任务失败 (v4.0)**: 当算子 ② 章节预处理抛出任何异常时，整个任务直接标记为 `failed`。章节归属是本系统的核心价值，无章节信息的抽取结果不满足产品目标，因此不做静默降级。如需在没有章节预处理的情况下运行，可以不配置章节预处理 LLM（此时任务会跳过章节预处理步骤，但题目将不含章节信息）。
...
### 5.3 算子 ②：ChapterPreprocess（全文推理，零筛选两轮 LLM 校验）

**职责**: 识别文档中所有章节标题及其 Block ID，构建全局章节结构。使用长上下文 LLM 通读全文，从全局视角推理原书真实目录。

**架构设计原则**:

章节目录推理是**语义推理任务**，不是模式匹配任务。代码/正则只能识别格式规范的标题，无法处理"无目录但有逻辑结构"的文档。只有全局视角才能判断章节连续性（发现断号、推断缺失层级）。因此，**全文推理是唯一正确的方法**，三轨混合方案已废弃。

**输入材料约束**:

本系统处理的输入为教育类 PDF（教材、试卷、书籍、练习册等）。MinerU 解析输出的 `content_list.json` 中 `text_level` 等字段不可信赖，可能出现标题错标、漏标、破碎（一个标题拆成多 block）、层级错误等情况。因此不能依赖 MinerU 的结构化信息，必须基于全文内容重新推理。

**推理目标**:
- 若原书有目录：推理出原书中真实存在的目录，包括标题名称、出现顺序、层级关系
- 若原书无目录（试卷/练习册等）：推理出该文档在逻辑上"应该存在"的结构，以文档中实际出现的结构性标题为依据，不得虚构

**处理流程**:

| 步骤 | 操作 | 说明 |
| :--- | :--- | :--- |
| **格式化全文** | 将 `FlatBlock[]` 转为紧凑文本，非标题文本截断至 50 字符，潜在标题保留 150 字符 | 降低 Token 消耗，同时保留关键内容 |
| **第一轮 LLM 抽取** | 将全文喂给长上下文 LLM，要求输出所有章节标题及其 Block ID | 零候选筛选，全文一次性推理 |
| **代码后处理** | 移除明显噪声条目（题型标签、教学标签、目录页条目等） | 过滤规则保守，宁可保留疑似标题 |
| **第二轮 LLM 自我校验** | 将初步结果反馈给 LLM，要求补全缺失、修复层级、检验连续性 | 确保章节编号连续无断号 |
| **构建 flat_map** | 代码根据 Block ID 计算每个章节的 `block_range` 和 `parent_id` | 为下游 ChapterMerge 提供索引 |

**超长文档处理**: 当全文 Token 估算超过模型上下文窗口时，自动切换分块处理模式。每个 Chunk 的 Prompt 中必须明确标注全局 Block ID 范围，LLM 必须输出全局 ID（而非 Chunk 内部相对序号）。

**成功标准**:
- 章节编号连续，无明显断号（如 Chapter 1、2、3 不得出现 Chapter 1→Chapter 3 的跳跃）
- 输出的每个 `block_id` 必须在 `FlatBlock[]` 的 ID 范围内
- 章节标题顺序与文档出现顺序一致

**失败处理**: 任何抛出的异常（包括上下文超长、API 错误、格式解析失败）均直接导致整个任务失败。不做降级回退。

**关键约束**:
- 必须将 LLM 的原始输出保存到 `debug/chapter_llm_round1_*.json` 和 `debug/chapter_llm_round2_*.json`
- 代码实现文件：`server/chapterPreprocess.ts`（`preprocessChapters` 函数）
- `server/chapterPreprocessV2.ts` 已废弃，移入 `archive/` 目录，不再调用

### 5.4 算子 ③：ChapterValidation

**职责**: 验证 `ChapterPreprocess` 的输出质量，或在验证失败时输出 `null` 以触发后续步骤。
...
**核心规则**:
- **当 `chapter_flat_map` 为 `null` 时（未配置章节预处理 LLM）**: 直接使用每道题目自带的 `chapter_title`，不做任何覆盖
...
┌─────────────────────────────────────────────────────┐
│                     后端 (Node.js)                   │
│  Express.js + tRPC (API) + Drizzle ORM (数据库)      │
│                                                      │
│  ┌─────────────────────────────────────────────┐    │
│  │       核心流水线 (7 个算子)                   │    │
...
> **v4.1 修订**: 修正后端框架为 Express.js（实际实现），而非 Hono。
...
路线图基于 v4.1（2026-02-21）的当前状态更新，标注各项完成情况。

### 8.1 已完成（v4.1 及之前）

| 编号 | 任务 | 完成版本 | 说明 |
| :--- | :--- | :--- | :--- |
| ✅ P0-1 | **ChapterValidation 算子** | v3.0 | 在 `chapterPreprocess.ts` 的两轮 LLM 校验流程中内置验证步骤 |
...
| ✅ P0-4 | **章节预处理回归全文推理** | **v4.0** | `taskProcessor.ts` 切换为 `preprocessChapters`（全文推理），移除优雅降级，章节预处理失败时任务直接失败；`chapterPreprocessV2.ts` 废弃并移入 `archive/` |
| ✅ P0-5 | **validateChapterEntries 导出** | **v4.0** | `chapterPreprocess.ts` 第699行，函数已添加 `export`，可作为独立算子调用 |
| ✅ P0-6 | **ChapterMerge fallback 策略修正** | **v4.0** | `extraction.ts` 第277-293行，fallback 改为预处理优先（preTitle） |
...
| ✅ P2-x | **项目版本对齐修订** | **v4.1** | 修正技术栈描述、数据库结构、API路由、文件路径约定，确保PRD与实际实现完全对齐 |
...
**版本号不一致状态 (v4.1)**:
- **package.json**: 仍为 `"version": "1.0.0"`（项目创建时的初始版本）
- **说明文档**: 已更新为 v4.0（基于PRD版本）
- **PRD文档**: 当前修订为 v4.1（基于实际对齐）
- **Git标签**: 最新为 v4.0（代码修复提交）

**建议**: 在下一发布周期中统一将 package.json 版本号升级至 4.1，保持全项目版本一致性。
...
| `page_processing_logs` | 页面处理记录（**实际用于日志记录，非断点恢复**） |
| `task_logs` | 任务实时日志（`level`/`stage`/`chunkIndex` 等字段） |
| `audit_logs` | 审计日志（合规性审查） |

> **v4.1 修订**: `page_processing_logs` 表实际用于记录分块处理日志，而非断点恢复。

**任务状态枚举**: `pending` / `processing` / `completed` / `failed` / `paused`

> **注意**: `paused` 状态在数据库 Schema 中已存在，但后端 `pauseTaskProcessing` 函数尚未实现实际暂停逻辑（v4.1 仍为空实现）。

---

## 11. 附录 C：API 接口 (tRPC 路由)

所有 API 通过 tRPC 提供，主要路由定义于 `server/routers.ts`。
...
> **v4.1 修订**: 修正路由描述，实际路径为 `server/routers.ts`，而非 `routers/` 目录。
...
│   ├── chapter_llm_round1_attempt1.json # 第一轮章节抽取 LLM 输出
│   ├── chapter_llm_round2_attempt1.json # 第二轮章节校验 LLM 输出
│   ├── chapter_prompt_round1.txt # 第一轮章节抽取 Prompt
│   ├── chapter_prompt_round2.txt # 第二轮章节校验 Prompt
│   ├── chunk_<N>_prompt.txt   # QuestionExtract 第 N 个 Chunk 的 Prompt
│   └── chunk_<N>_llm_output.txt # QuestionExtract 第 N 个 Chunk 的 LLM 响应
├── logs/
│   ├── extraction.log         # 流水线文本日志
│   ├── chunk_0_parsed_questions.log # 每个Chunk的解析日志
│   └── chunk_0_llm_output.log # 每个Chunk的LLM原始输出
└── results/
    ├── questions.json          # 最终结果（JSON 格式）
    └── questions.md            # 最终结果（Markdown 格式）
```

> **v4.1 修订**: 基于实际 `server/uploads/tasks/` 目录结构更新文件路径约定，包含所有实际生成的调试文件和日志文件。
...
> **v4.1 新增**: 本附录基于 v4.1 项目实际状态更新已知问题列表。

### F.1 功能性已知问题

| 编号 | 问题描述 | 严重程度 | 状态 | 规避方案 |
| :--- | :--- | :--- | :--- | :--- |
| F-001 | **暂停/取消任务为空实现**: `pauseTaskProcessing` 和 `cancelTaskProcessing` 仅打印警告，不实际暂停任务 | 中 | **待实现（P1-新增-1）** | 用户只能等待任务完成或在服务器端手动终止进程 |
| F-002 | **按 Block 数量切块存在 Token 超限风险**: 当单个 Block 内容很长时（如复杂表格），100 个 Block 可能超过模型的 Token 限制 | 中 | 待优化（P1-新增-2） | 适当减小 `MAX_CHUNK_SIZE`，或手动将超大文档拆分后上传 |
| F-003 | **全文推理分块模式的 ID 偏移风险**: 当文档过长需要分块处理时，如果 Prompt 未明确标注全局 ID，LLM 可能输出相对序号 | 低 | 已有防护（v2.x），待加强 | 查看 `debug/chapter_llm_round1_*.json`，手动核查 block_id 是否在合法范围内 |
| F-004 | **History 与 Tasks 页面功能重叠**: 两个页面均展示任务列表，差异化定位尚不清晰 | 低 | 待 UI 优化（P1-新增-3） | 了解区别：Tasks 含全部状态，History 仅含 completed |
...
| F-L002 | **复杂版面章节识别偏差**: 对于排版极不规范的文档（如手写扫描件、多栏布局），全文推理准确率可能低于 95%，导致任务失败 | 中 | 属于 LLM 能力上限，可通过更换更强的长上下文模型缓解 |
| F-L003 | **目录页噪声**: 部分文档的目录页格式特殊，LLM 可能将目录条目误识别为正文章节，需要第二轮校验过滤 | 低 | `chapterPreprocess.ts` 的代码后处理步骤包含目录条目噪声过滤规则 |
...
| 编号 | 限制描述 | 影响 | 说明 |
| :--- | :--- | :--- | :--- |
| F-P001 | **章节预处理增加总处理时间**: 全文推理需要将整个文档发送给长上下文 LLM，对于超大文档额外耗时较为显著 | 中 | 可通过仅配置题目抽取模型（不配置章节预处理模型）来跳过章节预处理阶段（但题目将不含章节信息） |
...
| 编号 | 限制描述 | 影响 | 说明 |
| :--- | :--- | :--- | :--- |
| F-C001 | **版本号不一致**: package.json 仍为 1.0.0，与其他文档版本不同步 | 低 | 建议在下一发布周期统一升级至 4.1 |
| F-C002 | **技术栈描述偏差**: PRD v4.0 中描述使用 Hono，实际使用 Express.js | 中 | 已在 v4.1 中修正 |
...
**教训**: 引入独立的章节预处理阶段（使用长上下文 LLM 通读全文），从全局视角构建章节映射，然后通过 ChapterMerge 将全局章节信息注入到每道题目中。这是"全文推理双 LLM 流水线"引入的根本原因。代码/正则的三轨混合方案（v3.0 尝试过）已证明不适合这个任务，因为章节目录推理是语义推理任务，只有全局视角才能保证章节连续性。
...
**背景**: v4.0 PRD 发布后，发现技术栈描述、数据库结构、文件路径等多个方面与实际代码存在差异，影响项目理解和后续开发。

**教训**: **PRD 是唯一标准文档，必须定期与实际代码对齐**。特别是在重大架构变更后，需要主动核查PRD中所有描述的准确性，包括技术栈、版本号、文件结构、API定义等。v4.1 修订专门解决了这个同步问题。